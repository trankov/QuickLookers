(() => {
  ...
  // node_modules/emoji-regex-xs/index.js
  var require_emoji_regex_xs = __commonJS({
    "node_modules/emoji-regex-xs/index.js"(exports, module) {
      var r3 = String.raw;
      var seq = r3`(?:\p{Emoji}\uFE0F\u20E3?|\p{Emoji_Modifier_Base}\p{Emoji_Modifier}?|\p{Emoji_Presentation})`;
      var sTags = r3`\u{E0061}-\u{E007A}`;
      module.exports = () => new RegExp(r3`[\u{1F1E6}-\u{1F1FF}]{2}|\u{1F3F4}[${sTags}]{2}[\u{E0030}-\u{E0039}${sTags}]{1,3}\u{E007F}|${seq}(?:\u200D${seq})*`, "gu");
    }
  });
  ...
  function resolveParsedThemeRules(parsedThemeRules, _colorMap) {
    parsedThemeRules.sort((a, b) => {
      ...
      r3 = strArrCmp(a.parentScopes, b.parentScopes);
      if (r3 !== 0) {
        return r3;
      }
    ...
    static _compileCaptures(captures, helper, repository) {
      ...
      if (captures) {
        ...
        for (const captureId in captures) {
          ...
          r3[numericCaptureId] = _RuleFactory.createCaptureRule(helper, captures[captureId].$vscodeTextmateLocation, captures[captureId].name, captures[captureId].contentName, retokenizeCapturedWithRuleId);
        }
      }
      return r3;
    }
  ...
  var CompiledRule = class {
    ...
    toString() {
      ...
      for (let i = 0, len = this.rules.length; i < len; i++) {
        r3.push("   - " + this.rules[i] + ": " + this.regExps[i]);
      }
      return r3.join("\n");
    }
  ...
  function _tokenizeString(grammar, lineText, isFirstLine, linePos, stack, lineTokens, checkWhileConditions, timeLimit) {
    ...
    function scanNext() {
      ...
      const r3 = matchRuleOrInjections(
        grammar,
        lineText,
        isFirstLine,
        linePos,
      ...
      const captureIndices = r3.captureIndices;
      const matchedRuleId = r3.matchedRuleId;
  ...
  function _checkWhileConditions(grammar, lineText, isFirstLine, linePos, stack, lineTokens) {
    ...
    for (let whileRule = whileRules.pop(); whileRule; whileRule = whileRules.pop()) {
      ...
      const r3 = ruleScanner.findNextMatchSync(lineText, linePos, findOptions);
      if (false) {
        console.log("  scanning for while rule");
        console.log(ruleScanner.toString());
      }
      if (r3) {
        const matchedRuleId = r3.ruleId;
        if (matchedRuleId !== whileRuleId) {
          stack = whileRule.stack.pop();
          break;
        }
        if (r3.captureIndices && r3.captureIndices.length) {
          lineTokens.produce(whileRule.stack, r3.captureIndices[0].start);
          handleCaptures(grammar, lineText, isFirstLine, whileRule.stack, lineTokens, whileRule.rule.whileCaptures, r3.captureIndices);
          lineTokens.produce(whileRule.stack, r3.captureIndices[0].end);
          anchorPosition = r3.captureIndices[0].end;
          if (r3.captureIndices[0].end > linePos) {
            linePos = r3.captureIndices[0].end;
            isFirstLine = false;
          }
        }
      } else {
  ...
  var Grammar = class {
    ...
    tokenizeLine(lineText, prevState, timeLimit = 0) {
      const r3 = this._tokenize(lineText, prevState, false, timeLimit);
      return {
    ...
    tokenizeLine2(lineText, prevState, timeLimit = 0) {
      const r3 = this._tokenize(lineText, prevState, true, timeLimit);
      return {
...
\f\r "&'/<=>`.split(""), "\0	\n\f\r \"&'/<=>`".split("")]
  ...
  var import_emoji_regex_xs = __toESM(require_emoji_regex_xs(), 1);
...
`.replace(/\s+/g, ""), "gsu");
  ...
  var namedCapturingDelim = r`\(\?<(?![=!])(?<captureName>[^>]+)>`;
  var token = new RegExp(r`${namedCapturingDelim}|${recursiveToken}|\(\?|\\?.`, "gsu");
  var overlappingRecursionMsg = "Cannot use multiple overlapping recursions";
  var emulationGroupMarkerRe = new RegExp(r`(?:\$[1-9]\d*)?${emulationGroupMarker.replace(/\$/g, r`\$`)}`, "y");
    ...
    while (match = token.exec(expression)) {
      ...
      if (m === "[") {
        ...
        if (rDepth) {
          ...
          if (hasNumberedBackref && hasUnescaped(groupContents, r`${namedCapturingDelim}|\((?!\?)`, Context.DEFAULT)) {
  ...
  function repeatWithDepth(expression, reps, namesInRecursed, direction, useEmulationGroups) {
    ...
    for (let i = 0; i < reps; i++) {
      ...
      result += replaceUnescaped(
        ...
        r`${namedCapturingDelim}|\\k<(?<backref>[^>]+)>${useEmulationGroups ? r`|(?<unnamed>\()(?!\?)(?:${emulationGroupMarkerRe.source})?` : ""}`,
        ({ 0: m, index, groups: { captureName, backref, unnamed } }) => {
          ...
          return captureName ? `(?<${captureName}${suffix}>${useEmulationGroups ? emulationGroupMarker : ""}` : r`\k<${backref}${suffix}>`;
        },
        Context.DEFAULT
      );
    }
...
`.replace(/\s+/g, ""), "gsu");
  ...
  function splitEscapedNumToken(token2, numCaptures) {
    ...
    for (let i = 0; i < matches2.length; i++) {
      ...
      if (i === 0 && m !== "8" && m !== "9") {
        value2 = parseInt(m, 8);
        if (value2 > 127) {
          throw new Error(r2`Octal encoded byte above 177 unsupported "${raw2}"`);
        }
      } else {
  ...
  var FirstPassVisitor = {
    ...
    VariableLengthCharacterSet({ node, replaceWith }, { accuracy, minTargetEs2024 }) {
      ...
      if (kind === AstVariableLengthCharacterSetKinds.newline) {
        ...
        const emoji = minTargetEs2024 ? r2`\p{RGI_Emoji}` : (0, import_emoji_regex_xs.default)().source.replace(/\\u\{/g, `\\x{`);
  ...
  function genCharacterSet({ kind, negate, value, key: key2 }, state) {
    ...
    if (kind === AstCharacterSetKinds.property) {
      ...
      return `${negate ? r2`\P` : r2`\p`}{${key2 ? `${key2}=` : ""}${value}}`;
    }
  ...
  function codeToTokens(internal, code, options) {
    ...
    if ("themes" in options) {
      ...
      if (defaultColor && !themes2.find((t) => t.color === defaultColor))
      ...
      fg = themes2.map((t, idx) => (idx === 0 && defaultColor ? "" : `${cssVariablePrefix + t.color}:`) + (applyColorReplacements(themeRegs[idx].fg, themeColorReplacements[idx]) || "inherit")).join(";");
      bg = themes2.map((t, idx) => (idx === 0 && defaultColor ? "" : `${cssVariablePrefix + t.color}-bg:`) + (applyColorReplacements(themeRegs[idx].bg, themeColorReplacements[idx]) || "inherit")).join(";");
