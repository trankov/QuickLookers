...
**5. Оптимизация показа (фаза 5)** — **реализовано** (спека `docs/superpowers/specs/2026-06-29-preview-display-optimizations-design.md`, план `docs/superpowers/plans/2026-06-29-preview-display-optimizations.md`, 8 задач). Четыре рычага: **перенос длинных строк** (всегда включён — минифицированный JSON больше не уезжает за край); **обрезка** до 2000 строк с плашкой «показаны первые N строк» (прокрутка в пределах N) + защита от гигантских файлов (ограниченное чтение префикса при размере >2 МБ); **тёплый общий `WKWebView`** на жизнь процесса (убирает выбросы 1–2,6 с холодного WebContent); **кэш готового HTML** в контейнере App Group (`Caches/html`, потолок 5 МБ, LRU по mtime файла). Ключ кэша — из атрибутов файла (путь+mtime+size) + язык+тема+потолок строк+версия бандла, без чтения содержимого; попадание минует чтение и движок. Чистая логика (обрезка, ключ/хранилище кэша, страница) — в `QuickLookersPreviewKit` под `swift test`; склейка с WebKit — в `PreviewViewController`. **Поведение на ошибке:** нечитаемый (не-UTF-8) файл бросает → системный дженерик-превью (как и прежде на маленьких файлах, теперь и на >2 МБ); ошибка кэша / нет контейнера App Group — показ идёт без кэша, не ломается. **Остаётся ручная проверка пользователем** (лог-спайк в Finder: warm=true, cache=false→true, нет выбросов).
