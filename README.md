...
Проект в активной разработке. Движок готов и несёт всю библиотеку Shiki (218 языков, 54 темы); расширение Preview перехватывает в Finder ~20 языков с подсветкой; главное приложение-менеджер имеет рабочее окно с тремя вкладками и записывает настройки в App Group — расширение Preview их уже читает.

| Подсистема | Статус |
|---|---|
| Движок рендеринга (`QuickLookersEngine`) | готов; вся библиотека Shiki (218 языков, 54 темы), встроенные грамматики |
| Расширение QuickLook Preview | перехват ~20 языков (проверенные системные UTI), тема из настроек |
