...
> **ОБНОВЛЕНИЕ 2026-07-04:** задача Pygments ВЫПОЛНЕНА (коммит `b99f2fd`). Итог и цифры —
> `2026-07-04-pygments-coverage-measurement.md`. Кратко: +403 dyn-расширения в перехват
> (экспортный UTI 654→1057), развязка перехвата от языка (ключ `interceptExtensions`).
> Оставшийся раздел ниже — исходная постановка; держим как контекст.
