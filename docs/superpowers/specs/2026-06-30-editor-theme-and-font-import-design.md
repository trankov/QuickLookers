...
**Риск и страховка.** Первой задачей плана — короткий спайк: скормить реальную
`SetiMonokai.tmTheme` (через plist→объект) движку и убедиться, что рисуется. Если не
выйдет — осознанный запасной вариант: на первом этапе импортировать только JSON-темы,
а `.tmTheme` мягко пропускать с понятным сообщением «формат темы пока не поддержан».
...
- `.tmTheme` не сконвертировалась → мягкий пропуск с понятным текстом (см. страховку Узла 4).
...
1. **Спайк `.tmTheme`** — подтвердить plist→объект→движок рисует (снять главный риск).
