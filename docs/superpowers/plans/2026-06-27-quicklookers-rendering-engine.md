...
- [x] **Task 6** — бенчмарк бюджета. cold≈440 мс / warm≈188 мс (release) на 200 строках — над ориентиром 100 мс. Замер + заметка `docs/superpowers/notes/2026-06-28-engine-benchmark.md`; нативный порт отложен в пользу более дешёвых рычагов.
...
- **Поправка плана (Task 3):** в тесте делимитер raw-строки темы `#"..."#` заменён на `##"..."##` — внутри JSON есть `#ffffff`, а последовательность `"#` досрочно закрывала строку.
- **Поправка плана (Task 4):** в shiki 1.x грамматики/темы поставляются как ESM `.mjs`, а не `.json` — `cp *.json` из плана невозможен. Вместо этого добавлен скрипт `js/extract-resources.mjs`: импортирует модули `@shikijs/langs|themes` и сериализует в JSON-ресурсы. Грамматика — массив регистраций, берём элемент с `name == id`. Это и есть документированный способ пополнения ресурсов.
- **Поправка плана (Task 6):** бюджет ~100 мс на голом движке не достигнут (warm≈188 мс на 200 строках, release ≈ debug → стоимость в JS-слое JSC). По решению пользователя тест оформлен как измеритель (печатает cold/warm + санити-потолок), а не жёсткие CI-ворота. Бюджет проверяем на полном конвейере показа (с кэшем HTML и обрезкой первого экрана). Нативный порт отложен. Детали — в заметке бенчмарка.
